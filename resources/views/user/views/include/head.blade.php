<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>Adda - Social Network HTML Template</title>
    <meta name="robots" content="noindex, follow" />
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <!-- Favicon -->
    <link rel="shortcut icon" type="image/x-icon" href="{{ url('assets/images/favicon.ico') }}">

    <!-- CSS
 ============================================ -->
    <!-- google fonts -->
    <link href="https://fonts.googleapis.com/css?family=Roboto:300,300i,400,400i,500,500i,700,700i,900"
        rel="stylesheet">
    <link rel="stylesheet" href="{{ url('user/css/mystyle.css') }}">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/vendor/bootstrap.min.css') }}">
    <!-- Icon Font CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/vendor/bicon.min.css') }}">
    <!-- Flat Icon CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/vendor/flaticon.css') }}">
    <!-- audio & video player CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/plugins/plyr.css') }}">
    <!-- Slick CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/plugins/slick.min.css') }}">
    <!-- nice-select CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/plugins/nice-select.css') }}">
    <!-- perfect scrollbar css -->
    <link rel="stylesheet" href="{{ url('user/assets/css/plugins/perfect-scrollbar.css') }}">
    <!-- light gallery css -->
    <link rel="stylesheet" href="{{ url('user/assets/css/plugins/lightgallery.min.css') }}">
    <!-- Main Style CSS -->
    <link rel="stylesheet" href="{{ url('user/assets/css/style.css') }}">
    {{-- Custom CSS --}}
    <link rel="stylesheet" href="{{ url('user/css/mystyle.css') }}">

    <link rel="stylesheet" type="text/css" href= {{url("login/css/fontawesome-all.min.css")}}>
    <link rel="stylesheet" type="text/css" href={{url("login/css/iofrm-style.css")}}>
    <link rel="stylesheet" type="text/css" href={{url("login/css/iofrm-theme10.css")}}>

</head>
